...
# screen values
onready var screen: Rect2 = get_tree().root.get_visible_rect()
onready var screenBox: BoundBox = BoundBox.new(screen)

# object instancing
onready var ball: Ball = Ball.new(screenBox.getCenter())
onready var playerPaddle = PlayerPaddle.new(screenBox)
onready var aiPaddle = AiPaddle.new(screenBox)
...
func checkCollisions() -> void:
	...
	if Collisions.pointToRectangle(ball.getPosition(), Rect2(playerPaddle.getRect())) and ball.isMovingLeft():
